...
{
	int nthreads, me, i, chunk,count=0;
	float a[N], b[N], c[N];
	...
	#pragma omp parallel shared(a,b,c,nthreads,chunk) private(i,me) firstprivate(count)
	{
		...
		for (i=0; i<N	; i++)
		{
			...
			printf("value of count is %i for the thread %i\n",count,me);
